...
> **Don't move the iris past the white of the eye.** It'll look weird. Keep it natural — a little goes a long way. If it does clip, see [Step 4: Clipping](#step-4-clipping-mask-keep-the-iris-inside-the-eye).
...
Exact same process as [Step 1](#step-1-leftright-eye-movement), just vertical:
...
3. If it looks off: with both parameters at those positions, adjust the iris with the [deformation tool](#where-things-are-in-cubism)
...
> **Two ways to do googly eyes.** This section covers the fun one — physics-based wobble where the eyes jiggle and settle after head movements. Skip to [Step 6](#step-6-googly-eyes--independent-control-route) for independent eye control instead.
...
> **Tip:** You can combine this with the [jiggle physics from Step 5](#step-5-googly-eyes--the-jiggle-physics-route) — independent parameters + physics = each eye jiggles on its own pendulum. Maximum chaos.
...
|:---|:---|:---|
| Iris teleports instead of sliding | Used Move tool instead of [Deformation tool](#where-things-are-in-cubism) | Switch to deformation tool (lasso icon, [Step 1](#step-1-leftright-eye-movement)) |
| Iris goes outside the eye | No [clipping mask](#step-4-clipping-mask-keep-the-iris-inside-the-eye) set up | Add clipping ([Step 4](#step-4-clipping-mask-keep-the-iris-inside-the-eye)) |
| Both eyes move even when I only select one | Both irises on same mesh | Make sure each iris is a separate layer/mesh |
| Corners look stretched | X and Y deformations stacking badly | Manually adjust the [corner keyforms](#step-3-check-the-corners) |
| Eyes look dead/robotic | Not enough movement range | Increase how far you shift the iris at -1 and +1 |
| Jiggle never stops | Damping too low | Raise [damping](#step-5-googly-eyes--the-jiggle-physics-route) from 0.2 → 0.4 |
| No jiggle at all | Damping too high, or input not connected | Lower damping, check [physics input](#setting-up-jiggle-on-the-iris) is linked to head params |
...
3. **[Head & Body](live2d-head-body-rigging-guide-v3.md)** — `ParamAngleX/Y/Z` (the big one — makes tracking feel alive)
4. **[Physics, Hair & Accessories](live2d-physics-hair-guide-v3.md)** — same jiggle setup as googly eyes, just on hair layers
